...
date: "Updated on 10 February 2023"
output: 
  html_document:
    toc: true
    toc_depth: 2
...
##### Bookmark 6 September 20222 #####
# I should add all the figure and table captions from the manuscript 
...
# Data
The data have been uploaded to Figshare. They can be accessed using the follwing DOI link [https://doi.org/10.6084/m9.figshare.15088014](https://doi.org/10.6084/m9.figshare.15088014). The datafile necessary is "RBFW3_Anon_Data_210720.csv". This datafile is also available in the [fairywren-nonbreeding-behavior](https://github.com/treyhendrix/fairywren-nonbreeding-behavior) GitHub repository in the "Data" folder. The contents of these data are detailed in the description on Figshare and in a metadata file in the GitHub repository. The structure of the data frame is: 
```{r Data Path}
...
file_path_to_RBFW_csv <- paste0(project_file_path, "/Data/RBFW3_Anon_Data_210720.csv")
```

```{r Data frame structure, include = TRUE}
RBFW <- read_csv(file_path_to_RBFW_csv)
glimpse(RBFW)
```

# Logistic Mixed-effects Models 
In our analysis, we use logistic mixed-effects models with a binomial distribution and logit link to determine which factors (plumage category, age, and day of year) predict whether or not a behaviour of interest (allopreening, chasing, courtship, preening, and vocalizing) was observed during an observation.
...
## Table S2
#### Bookmark 10 February 2023 ####
Need to transfer this code from other scripts and import the opportunistic dataset 
...
```{r Saving histogram proportion figure, include = FALSE}
ggsave(paste0(output_file_path, "/Figure_S4.pdf"), plot = figure_s4)
```


## Figure S5
```{r Proportion of all observations containing focal behaviors}
...
```{r Saving all behavior proportion plot, include = FALSE}
ggsave(paste0(output_file_path, "/Figure_S5.pdf"), plot = figure_s5, width = 7, height = 5)
```

## Figure S6
#### Bookmark 2 October 2022 #### 
I need to add heat map figure code  
